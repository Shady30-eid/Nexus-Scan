...
func (d *DB) Migrate() error {
        migrations := []string{
                ...
                `CREATE TABLE IF NOT EXISTS malware_package_names (
                        id             INTEGER PRIMARY KEY AUTOINCREMENT,
                ...
                `CREATE INDEX IF NOT EXISTS idx_malware_pkgnames_pkg      ON malware_package_names(package_name);`,
                `CREATE INDEX IF NOT EXISTS idx_malware_pkgnames_family   ON malware_package_names(malware_family);`,
                `CREATE INDEX IF NOT EXISTS idx_malware_pkgnames_severity ON malware_package_names(severity);`,
...
func (d *DB) Seed() error {
        if err := d.seedHashSignatures(); err != nil {
                return err
        }
        return d.seedPackageNames()
}

func (d *DB) seedHashSignatures() error {
        var count int
        ...
        seeds := []struct{ name, hash, family, severity, desc string }{
                {"BankBot.A", "a3f5c8d2e1b4f9a6c7d0e3b2a5f8c1d4e7b0a3f6", "BankBot", "CRITICAL", "Fake banking overlay steals credentials and intercepts SMS OTP."},
                ...
                {"Rootnik.F", "f8c0b3e6a9d2f5c8b1e4a7d0c3f6b9e2a5d8c1f4", "Rootnik", "CRITICAL", "Local privilege escalation exploit, installs persistent root backdoor."},
                {"GhostPush.G", "a9d1c4f7b0e3a6d9c2f5b8e1d4a7c0f3b6e9d2a5", "GhostPush", "HIGH", "Silently installs arbitrary APKs abusing accessibility services."},
                {"Lotoor.I", "c1f3e6b9d2a5c8f1e4b7d0a3f6c9e2b5d8a1c4f7", "Lotoor", "CRITICAL", "Exploits CVE-2012-0056 and others to gain root access."},
                {"DroidDream.J", "d2a4f7c0e3b6d9a2f5c8e1b4d7a0f3c6e9b2d5a8", "DroidDream", "CRITICAL", "Early Google Play rootkit — first major Play Store malware campaign."},
        }
...
func (d *DB) seedPackageNames() error {
        var count int
        if err := d.conn.QueryRow(`SELECT COUNT(*) FROM malware_package_names`).Scan(&count); err != nil {
                return err
        }
        if count > 0 {
        ...
        d.log.Info("seeding malware package name database", "entries", len(KnownMalwarePackages))
        ...
        stmt, err := tx.Prepare(`INSERT OR IGNORE INTO malware_package_names(package_name,malware_name,malware_family,severity,description,created_at) VALUES(?,?,?,?,?,?)`)
        if err != nil {
                return err
        }
        defer stmt.Close()
        ...
        now := time.Now()
        for _, s := range KnownMalwarePackages {
                if _, err := stmt.Exec(s.PackageName, s.Name, s.Family, s.Severity, s.Desc, now); err != nil {
                        return err
                }
        }
        ...
        d.log.Info("malware package name database seeded", "entries", len(KnownMalwarePackages))
        return nil
}

// LookupPackageName checks the malware_package_names table for a known malicious package.
func (d *DB) LookupPackageName(packageName string) (*models.MalwareSignature, error) {
        row := d.conn.QueryRow(
                `SELECT id, malware_name, package_name, malware_family, severity, description, created_at
                 FROM malware_package_names WHERE package_name = ? LIMIT 1`, packageName)

        var sig models.MalwareSignature
        var pkgName, createdAt string
        err := row.Scan(&sig.ID, &sig.MalwareName, &pkgName, &sig.MalwareFamily, &sig.Severity, &sig.Description, &createdAt)
        if err == sql.ErrNoRows {
                return nil, nil
        }
        if err != nil {
                return nil, fmt.Errorf("lookup package name: %w", err)
        }
        sig.SHA256Hash = pkgName
